...
void runSuccess() {
    char16_t s1[4]; s1[0] = L'A'; s1[1] = L'A'; s1[2] = L'A'; s1[3] = L'\0';
    char s2[] = "BBB";
    //@ assert \valid(s1+(0..3));
    //@ assert \exists integer x; s1[x] == L'\0';
    //@ assert \valid(s2+(0..\block_length(s2)-1));
    //@ assert \exists integer x; s2[x] == '\0';
    mbrtoc16(s1, s2, 3, anystate());
    mbrtoc16(NULL, s2, 3, anystate());
}

void runSuccess1() {
    mbrtoc16(NULL, NULL, anysize(), anystate());
    mbrtoc16(NULL, NULL, anysize(), NULL);
}
...
void testValues() {
    ...
    result = mbrtoc16(s1, s2, 3, anystate());
